...
public class ServerHandler extends IoHandlerAdapter {
	...
	public ServerHandler(ProtocolEnum pEnum, SessionManager _manager) {
	...
	public void sessionOpened(IoSession session) throws Exception {
		...
		logger.debug("开启连接：" + session.getId() + ":" + df.format(new Date()) + session.getAttribute("ID") + session);
	}
	...
	public void sessionClosed(IoSession session) throws Exception {
		...
		logger.warn(
				"关闭链接：" + session.getId() + ":" + df.format(new Date()) + session.getAttribute("ID") + "--" + session);
	...
	public void exceptionCaught(IoSession session, Throwable cause) {
		...
		logger.warn("錯誤：" + session.getId() + ":" + df.format(new Date()) + session.getAttribute("ID") + session);

		logger.debug("exceptionCaught", cause);

		// SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		// new AychWriter("发生异常关闭链接：" + session.getAttribute("ID") +
	...
	public void sessionIdle(IoSession session, IdleStatus arg1) throws Exception {
		...
		logger.warn("超时关闭链接：" + session.getId() + ":" + session.getAttribute("ID") + df.format(new Date()) + "--"
				+ session);	
		this.manager.removeSession(session);
		session.close(true);
	}
