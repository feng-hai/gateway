...
//				+ config.getKafkaGroupID());
		...
		// 把原始数据插入kafka中
		SendDataTokafka sendData = new SendDataTokafka( sendQueue);
		sendData.setDaemon(true);
		sendData.start();

		// 消费者--从kafka中读取数据，并把数据存入队列中
		CommandConsumer commandConsumer = new CommandConsumer( cmdQueue);
		commandConsumer.run();
		//
...
//		 ServerMainThread(20291,ProtocolEnum.P3G,
//		 sendQueue, vehicles,sessionManager);
//		 smt.start();
...
//		 ProtocolEnum.P808, sendQueue, vehicles,sessionManager);
//		 smt808.start();
